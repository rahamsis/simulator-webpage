...
function classVariants(baseClass: string, variants: Record<string, string>, selectedVariant?: string) {
  return `${baseClass} ${selectedVariant ? variants[selectedVariant] || "" : ""}`.trim();
}
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={clsx(sheetVariants( side ), className)} {...props}>
      {children}
      <SheetClose className="absolute right-4 top-4 opacity-70 transition-opacity hover:opacity-100 focus:ring-2">
